...
TEXTURE3D(unity_ProbeVolumeSH); SAMPLER(samplerunity_ProbeVolumeSH);
...
{
	float3 uvw = reflect(-surfaceWS.viewDirection, surfaceWS.normal);//使用视线的反射向量采样反射探针
...
{
	GI gi;
	// indirect lighting diffuse 间接光漫反射
...
#if defined(_SHADOW_MASK_ALWAYS)//总是使用shadowmask
